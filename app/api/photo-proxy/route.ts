...
/**
 * GET /api/photo-proxy?url=ENCODED_URL
 *
 * Google Places の写真 URL をサーバー側でフェッチして画像バイトを返す。
 * - places.googleapis.com/v1/.../media 形式 → skipHttpRedirect でCDN URLに解決してリダイレクト
 * - lh3.googleusercontent.com 等の公開CDN URL → そのままリダイレクト
 * - その他 → サーバーサイドフェッチして転送
 *
 * ブラウザからAPIキーを隠し、HTTP referrer 制限を回避する。
 */
export async function GET(req: NextRequest) {
  ...
  // ── places.googleapis.com/v1/.../media 形式 ──────────────────────────────
  // サーバー側で CDN URL（lh3.googleusercontent.com）に解決して 302 リダイレクト
  if (targetUrl.includes("places.googleapis.com/v1/") && targetUrl.includes("/media")) {
    ...
    try {
      // URL から photo name を抽出（クエリを除く）
      const urlObj = new URL(targetUrl);
      const photoPath = urlObj.pathname; // /v1/places/.../photos/.../media
  ...
  // ── 公開 CDN URL（lh3.googleusercontent.com 等）→ そのままリダイレクト ──
  if (
